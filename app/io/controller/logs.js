...
module.exports = app => {
    class LogsController extends app.Controller {
        ...
        async log() {
            ...
            let filename = `./app/logs/${name}/common/${name}.log.${moment().format('YYYY-MM-DD')}`;
            if (ctx.msg.level === 'error') {
                filename = `./app/logs/${name}/error/errors.log.${moment().format('YYYY-MM-DD')}`;
            }
